      ...
      <div className="px-8 py-6 bg-[#111111] border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">External data source for parameterization</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-8 max-w-4xl">
          ...
          <div className="bg-[#111111] rounded-xl p-6 border border-white/5 shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <p className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Name</p>
                <div
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {data.variable_names
                  ...
                  .map((name: string, idx: number) => (
                    <div
                      key={name.trim() || idx}
                      className="bg-yellow-400/10 border border-yellow-400/20 rounded-lg px-3 py-1.5 text-xs font-mono text-yellow-400"
                    >
                      {name.trim() ? '{{' + name.trim() + '}}' : ''}
                    </div>
                  ))
          ...
          <div className="bg-blue-500/5 border border-blue-500/10 rounded-xl p-6">
            <div className="flex gap-3">
              ...
              <div className="text-xs text-zinc-400 leading-relaxed space-y-3">
                <div>
                  <strong className="text-zinc-200">Local:</strong> In browser mode, the file picker usually returns
                  only the file name. Copy and paste the full CSV/TXT path if you plan to run this script locally.
                </div>
                <div>
                  <strong className="text-zinc-200">Distributed:</strong> Use only the file name or a relative path (for
                  example, <code className="text-zinc-300">users.csv</code>). Relampo handles path resolution
                  automatically across distributed nodes.
                </div>
              </div>
            </div>
          </div>
