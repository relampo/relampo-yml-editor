      ...
      */}
      {entries.length > 0 && (
        <div className="flex gap-2 px-2 pb-1">
          <div className="flex-1 min-w-[30%] text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
            {keyPlaceholder}
          </div>
          <div className="flex-[2] text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
            {valuePlaceholder}
          </div>
      ...
      {entries.map(([k, v], idx) => (
        <KeyValRow
          ...
          validationError={
            entries.filter(([otherK]) => otherK === k).length > 1
              ? 'Duplicate key'
              : validateKey
                ? validateKey(k) || undefined
                : undefined
          }
        />
      ))}
      ...
      {entries.length === 0 && readOnly && <div className="text-xs text-zinc-600 italic px-2">No items</div>}
    </div>
  );
}
