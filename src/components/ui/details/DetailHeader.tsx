...
export function DetailHeader({ title, subtitle, icon: Icon, onDuplicate, onDelete, className }: DetailHeaderProps) {
  const { t } = useLanguage();
  return (
    <div
      className={cn(
        'flex items-center justify-between px-4 py-2 border-b border-white/5 bg-[#111111] h-10 flex-shrink-0',
        className,
      )}
    >
      <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider truncate">{title}</h3>
          {subtitle && (
            <>
