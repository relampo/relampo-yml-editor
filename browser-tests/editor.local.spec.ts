...
test('standalone mocked Run and Debug streams render terminal states', async ({ page }) => {
  ...
  await page.route('**/api/run/browser-run/events**', route =>
    route.fulfill({
      ...
      body: [
        ...
        'event: done\ndata: {"status":"completed","error":null,"summary":{"report_schema_version":2,"status":"completed","transactions_configured":true,"overview":{"rps":1,"tps":2,"tps_status":"available","failure_percent":0,"completed_transactions":2,"incomplete_transactions":0},"test_name":"browser","start_time":"2026-08-18T00:00:00Z","end_time":"2026-08-18T00:00:01Z","duration":1000000000,"total_requests":1,"total_failures":0,"executed_vus":1,"metadata":{"configured_vus":"2"},"transactions":[{"name":"Smoke","count":2,"completed":2,"incomplete":0,"failures":0}],"node_resources":[{"node":"local","mem_peak_mb":12,"cpu_peak":12.5,"go_peak":7,"measurements":{"rss_peak_mib":64,"go_heap_peak_mib":12,"cpu_percent_peak":12.5,"goroutines_peak":7}}],"requests":[]}}\n\n',
      ].join(''),
    }),
  );
  ...
  await expect(page.getByText('RSS Peak', { exact: true }).locator('..').getByText('64 MiB', { exact: true })).toBeVisible();
  await expect(page.getByText('CPU Peak', { exact: true }).locator('..').getByText('12.5%', { exact: true })).toBeVisible();
  await expect(page.getByText('Goroutines Peak', { exact: true }).locator('..').getByText('7', { exact: true })).toBeVisible();
